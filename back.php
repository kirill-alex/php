...
{
    if (abs(cos($x)) < 1e-12) throw new Exception('tan не определён — деление на ноль');
    return tan($x);
}
function calcAsin(float $x): float
{
    if ($x < -1.0 || $x > 1.0) throw new Exception('asin не определён — аргумент вне [-1; 1]');
    return asin($x);
}
function calcAcos(float $x): float
{
    if ($x < -1.0 || $x > 1.0) throw new Exception('acos не определён — аргумент вне [-1; 1]');
    return acos($x);
}
function calcAtan(float $x): float { return atan($x); }
